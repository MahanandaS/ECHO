...
export default function LandingPage({ posts, isAuthenticated = true }) {
  const featured = isAuthenticated ? (posts.find((p) => p.featured) || posts[0]) : null;
  const latest = isAuthenticated ? posts.filter((p) => p.id !== featured?.id).slice(0, 6) : [];
